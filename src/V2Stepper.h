...
namespace V2Stepper {
  ...
  public:
    ...
    constexpr Driver(SPIClass* spi, uint8_t pinSelect, uint8_t pinStep) : _bus({.spi{spi}, .pin{pinSelect}}), _pinStep(pinStep) {}
  ...
  public:
    ...
    constexpr Motor(const Config conf, V2Base::Timer::Periodic* timer, SPIClass* spi, uint8_t pinSelect, uint8_t pinStep) :
    ...
    void setPosition(float position, float speedFraction = 1, void (*handler)() = NULL);
  ...
  private:
    ...
    struct {
      ...
      bool getReverse(uint32_t position) {
        auto distance{int32_t(position) - int32_t(now)};
        return distance < 0;
      }
    } _position{};
